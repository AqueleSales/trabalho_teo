...
try:
    ...
    print(f"Aviso: Erro ao iniciar Firebase Admin. Detalhes: {e}")
...
def moderar_texto():
    ...
    if not chave_api:
        return jsonify({'status': 'erro', 'msg': 'API Key ausente no Render.'})
    ...
    try:
        ...
        if "BLOQUEADO_SPAM" in texto_limpo: return jsonify({'status': 'bloqueado_spam'})
        if "BLOQUEADO_ODIO" in texto_limpo: return jsonify({'status': 'bloqueado_odio'})

        # PYTHON É QUEM SALVA AGORA
